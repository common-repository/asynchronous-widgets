...
add_action('wp_enqueue_scripts', 'aw_cond_output_js_loader');
/**
...
function aw_cond_output_js_loader(){
     ...
     */
    wp_register_script( "async-widgets", plugins_url( 'js/async-widgets.js' , __FILE__ ), array('jquery') );
    wp_enqueue_script( "async-widgets" );
    wp_localize_script( "async-widgets" , "async_widgets",
            array(
...
function async_widgets_get_callback(){
    ...
    // restore GET and POST from the main page
    // for plugins that use them.
    $_GET += $_SESSION['aw_get_params'];
    $_POST += $_SESSION['aw_post_params'];
...
class AsyncWidget {
    ...
    public function get_widget_content($widget_id){
        ...
        if( isset( $_SESSION['aw_'.$widget_id]['params'] ) ){
            ...
            /*
             * If Super Cache is enabled, we don't have
             * the parameters in the session, and have
             * to load them from the DB instead.
             */
    ...
    public function setup_widget_callback_intercept(){
        ...
        foreach($widgetids as $id){
            ...
            if( $this->widgetstore[$id]['callback'] !== $callback ){
            ...
            else if ( isset( $this->widgetstore[$id]['callback_wc_redirect'] )){
                unset( $this->widgetstore[$id]['callback_wc_redirect'] );
                $need_save = true;
            }
        }
        ...
        //store the GET and POST params, in case
        //a plugin is relying on those (we'll restore
        //them in the AJAX call
        $_SESSION['aw_get_params'] = $_GET;
        $_SESSION['aw_post_params'] = $_POST;
    }
    ...
    function async_widget_proxy_callback(){
         ...
         * Store things.
         *
         * If the page is using the Super Cache
         * plugin, we will have to store things
         * more persistently, because in Super Cache
         * delivered pages, the code doesn't go through
         * this hook anymore -- only if the page isn't
         * in the cache yet.
         * So put it in the DB. Can't say I'm enthousiastic
         * about this, but I don't see any other way. Super
         * Cache and this plugin are kind of conflicting,
         * anyway.
         */
        if( $GLOBALS['super_cache_enabled'] ){
            $this->widgetstore[$id]['params'] = $args;
            $this->save_widget_store();
        }
        else{
            //clean up
            if( isset( $this->widgetstore[$id]['params'] ) ){
                unset( $this->widgetstore[$id]['params'] );
                $this->save_widget_store();
            }
...
function aw_admin_init(){
    ...
    wp_register_style('aw_admin_styles', plugins_url( "css/async-widgets-admin.css", __FILE__ ) );
}
